...
- (void)layoutPreviewLayer {
    if (self.parentView != nil) {
        ...
        self.customPreviewLayer.position = self.parentView.center;
...
- (void)setPointOfInterestInParentViewSpace:(CGPoint)parentViewPoint {
    ...
    //    NSArray *captureDevices = [AVCaptureDeviceDiscoverySession devices];
    ...
    if (canSetFocus) {
        // Auto-Focus on selected point
        captureDevice.focusMode = AVCaptureFocusModeAutoFocus;
        captureDevice.focusPointOfInterest = focusPoint;
    }
    
    if (canSetExposure) {
        // Auto-Expose on selected point
